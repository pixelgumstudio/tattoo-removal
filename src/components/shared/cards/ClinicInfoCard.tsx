  ...
  return (
    <div className="rounded-lg shadow-lg bg-white dark:bg-zinc-900 p-4 max-w-md w-full">
      <div className="flex items-start gap-4">
        ...
        <div>
          <h3 className="font-semibold text-lg text-zinc-800 dark:text-white">{data.name}</h3>
          <Paragraph size='sm' className=" text-zinc-600 dark:text-zinc-300">
            {data.full_address}
          </Paragraph>
          <Paragraph size='sm' className=" mt-1 text-zinc-600 dark:text-zinc-300">📞 {data.phone}</Paragraph>
          <Paragraph size='sm' className=" text-zinc-600 dark:text-zinc-300">💵 $500 – $2000 · ⭐ {data.reviews}</Paragraph>
        </div>
      </div>
