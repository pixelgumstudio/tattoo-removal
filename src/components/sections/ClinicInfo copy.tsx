// app/clinic/page.tsx (or where this component lives)
...
import ClinicInfoSkeleton from "../shared/loader/ClinicInfoSkeleton";
import TextBlockSkeleton from "../shared/loader/TextBlockSkeleton";
import OpeningHoursSkeleton from "../shared/loader/OpeningHoursSkeleton";


// Lazy load all heavy components
const ClinicInfoCard = dynamic(() => import("@/components/shared/cards/ClinicInfoCard"), {
  loading: () => <ClinicInfoSkeleton />,
  ssr: false,
});
const OpeningHours = dynamic(
  () =>
    import("@/components/shared/OpeningHours").then((mod) => mod.OpeningHours),
  {
    ssr: false,
    loading: () => <OpeningHoursSkeleton />,
  }
);const MapEmbed = dynamic(() => import("@/components/shared/MapEmbed").then((mod) => mod.MapEmbed), {
  ssr: false,
});

const TextBlock = dynamic(() => import("@/components/shared/TextBlock").then((mod) => mod.TextBlock), {
  loading: () => <TextBlockSkeleton />,
  ssr: false,
});
const DescriptionCard = dynamic(() => import("@/components/shared/cards/DescriptionCard"), {
  loading: () => <DescriptionCardSkeleton />,
  ssr: false,
});
...
export default function ClinicPage({ store }: ClinicPageProps) {
  const { data, isLoading } = useSuggestByCities(`city=${store.city}&shuffle=true&min=3&limit=3`);
    ...
    <section className="w-full px-4 md:px-6 lg:px-10 py-12">
      <div className="w-full max-w-[1152px] mx-auto grid gap-8 lg:grid-cols-[2fr_1fr]">
        ...
        <aside className="space-y-6">
          <Suspense fallback={<ClinicInfoSkeleton />}>
